...
from reconstruct import dist_util
...
from reconstruct.script_util import (
    NUM_CLASSES,
...
def main(conf: conf_mgt.Default_Conf):
    ...
    for batch in iter(dl):
        ...
        n_gt=(result.get('gt') )
        n_gt=n_gt.cpu().numpy()
        # np.save(f"./log_iso_cosine/{batch['GT_name'][0]}_gt",n_gt)
        ...
        # np.save(f"./log_iso_cosine/{batch['GT_name'][0]}_masked", n_masked)

        n_mask = model_kwargs.get('gt_keep_mask')
        n_mask = n_mask.cpu().numpy()
        # np.save(f"./log_iso_cosine/{batch['GT_name'][0]}_mask", n_mask)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--conf_path', type=str, required=False, default="confs/diff.yml")
