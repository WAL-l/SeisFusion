...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return arr[np.newaxis,:], out_dict
...
def random_crop_arr(pil_image, data_size, min_crop_frac=0.8, max_crop_frac=1.0):
    min_smaller_dim_size = math.ceil(data_size / max_crop_frac)
    max_smaller_dim_size = math.ceil(data_size / min_crop_frac)
    ...
    crop_y = random.randrange(arr.shape[0] - data_size + 1)
    crop_x = random.randrange(arr.shape[1] - data_size + 1)
    return arr[crop_y : crop_y + data_size, crop_x : crop_x + data_size]
